...
class PowerUp extends PositionComponent with CollisionCallbacks, HasGameReference {
  ...
  void update(double dt) {
    ...
    bob += dt * 2.2;
    // bob senoidal correto (antes: bob%2==0 nunca true)
    position.y = _baseY + 4 * math.sin(bob * 2.2);
    lifeTime -= dt;
  ...
  void render(Canvas canvas) {
    ...
    final tp = TextPainter(
      text: TextSpan(text: emoji, style: const TextStyle(fontSize: 16, shadows: [Shadow(color: Colors.black38, blurRadius: 4)])),
...
class TurboEffect extends Component with HasGameReference {
  ...
  @override
  void update(double dt) {
    final player = _player ?? (parent is Player ? parent as Player : null);
    if (player == null) {
      if (parent is Player) _player = parent as Player;
      if (_player == null) return;
      // visual lazy add if onMount missed
      if (_visual == null) {
        _visual = _TurboVisual();
        _player!.add(_visual!);
      }
    ...
    _t += dt;
    // boost com cap — antes *=1.02 exponencial 115x em 4s
    const maxTurboSpeed = 360.0;
    _player!.velocity.x = (_player!.velocity.x * 1.015).clamp(-maxTurboSpeed, maxTurboSpeed);
    if (_player!.velocity.x.abs() < 20) {
      _player!.velocity.x += _player!.facing * 20;
    }
    if (_t >= duration) {
...
class _TurboVisual extends PositionComponent {
  ...
  @override
  void render(Canvas canvas) {
    final alpha = (0.5 + 0.5 * math.sin(_t * 6)).clamp(0.0, 1.0);
    canvas.drawRRect(
